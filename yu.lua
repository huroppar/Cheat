...
local followDistance, followInterval = 5, 0.1
...
playerTab:CreateSlider({
    ...
    Increment = 1,
    Suffix = "WalkSpeed",
    CurrentValue = speedDefault,
    Flag = "SpeedSlider",
    Callback = function(val) desiredSpeed = val end
})
playerTab:CreateToggle({
    Name = "Infinite Jump",
    CurrentValue = false,
...
playerTab:CreateToggle({
    ...
    CurrentValue = false,
    Flag = "WallClip",
    Callback = function(val) wallClipEnabled = val; setWallClip(val) end
})
playerTab:CreateToggle({
    Name = "AirTP Button Visible",
    CurrentValue = true,
    Flag = "AirTPVisible",
    Callback = function(val) airTPVisible = val; airTPBtn.Visible = val end
})
...
playerTab:CreateColorPicker({
    ...
    Flag = "AccentColor",
    Callback = function(color) Rayfield:ChangeAccentColor(color) end
})
...
local function makePlayerEntry(p)
    ...
    spawn(function()
        local ok, thumb = pcall(function() return Players:GetUserThumbnailAsync(p.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48) end)
...
local function refreshPlayerList()
    ...
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl ~= player then
            ...
            entry.Parent = workspace -- ダミー Parent
...
Players.PlayerRemoving:Connect(function(pl)
    refreshPlayerList(); if selectedEnemy == pl then selectedEnemy = nil end
end)
refreshPlayerList()
...
icon.Text, icon.BackgroundColor3, icon.Visible = "🐟", Color3.fromRGB(0,170,255), false
icon.Parent = player:WaitForChild("PlayerGui")
icon.MouseButton1Click:Connect(function() Window:Toggle(); icon.Visible=false end)
Window.CloseCallback = function() icon.Visible = true end

--================ 内部処理 =================
spawn(function() while true do local _,hum=getCharacter(); if speedEnabled and hum then pcall(function() hum.WalkSpeed=desiredSpeed end) end; wait(0.1) end end)
