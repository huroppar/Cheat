...
-- 🔧 設定
local Remote = game.ReplicatedStorage:WaitForChild("AttackEvent")  -- ←君のゲームのRemoteEvent名
local enabled = false
local fireRate = 10 -- 秒間10回（後でスライダーで変える）

-- 🧨 連射ループ
task.spawn(function()
    while task.wait(1 / fireRate) do
        if enabled then
            pcall(function()
                Remote:FireServer()
            end)
...
-- 🪟 Rayfield UIへ追加
local Tab = Window:CreateTab("Attack Tester", 4483362458)
