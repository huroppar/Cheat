...
playerTab:CreateSlider({Name="Speed Amount", Range={speedMin,speedMax}, Increment=1, CurrentValue=speedDefault, Callback=function(val) desiredSpeed=val end})
playerTab:CreateToggle({Name="Infinite Jump", CurrentValue=false, Callback=function(val) infiniteJumpEnabled=val end})
playerTab:CreateToggle({Name="WallClip", CurrentValue=false, Callback=function(val) wallClipEnabled=val; setWallClip(val) end})
...
playerTab:CreateToggle({Name="AirTP Button Visible", CurrentValue=true, Callback=function(val)
    airTPVisible=val
    airTPBtn.Visible=val
end})
playerTab:CreateColorPicker({Name="Accent Color", Default=Color3.fromRGB(0,170,255), Callback=function(color) Rayfield:ChangeAccentColor(color) end})

-- 戦闘タブ
local combatTab = Window:CreateTab("Combat",4483362458)
local sectionPlayers = combatTab:CreateSection("Players")
local playerButtons = {}
...
local function makePlayerEntry(p)
    ...
    entry.Parent=sectionPlayers -- RayFieldセクションに追加
    return entry
end
...
combatTab:CreateToggle({Name="Follow Selected", CurrentValue=false, Callback=function(val)
    followBtnVal=val
    followTarget=val and selectedEnemy or nil
end})

combatTab:CreateSlider({Name="Attack Range", Range={1,50}, Increment=1, CurrentValue=10, Callback=function(val) attackRange=val end})
combatTab:CreateToggle({Name="Show Hitbox", CurrentValue=false, Callback=function(val) showHitbox=val end})
...
Window.CloseCallback=function() icon.Visible=true end
